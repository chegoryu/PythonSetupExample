...
Как можно заметить `setup.py` скрипт представляет собой вызов специальной функции из `setuptools` с правильными параметрами. Разберем их подробнее.
...
Первый это `name` которое задает общее имя нашего приложения. На него наложены некоторые ограничения, а именно - 'name' должно состоять только из английских букв, цифр, символов `_`, `-`.
...
Версия же нам не так важна, поэтому всегда можно просто указывать `'1.0'`, переустановки приложения с такой же версией проходят штатно.
