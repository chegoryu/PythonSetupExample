...
Как можно заметить основное отличие это `.pyw` вместо `.py`. Это специально расширение файла которое означает запуск `python` скрипта без открытия консоли (играет роль только при запуске из проводника, при запуске из консоли сама консоль будет оставаться на месте).
